...
运行方式：
    pytest project-1/test_csv_analyzer.py -v
"""
...
from csv_analyzer import (
    load_students, count_total, count_by_country,
    bet_completion_rate, build_report, save_report,
)
...
def test_completion_rate_empty():
    empty = pd.DataFrame(columns=["name","email","joined_date","country","bet_status"])
    assert bet_completion_rate(empty)["completion_rate_pct"] == 0.0

def test_completion_rate_all_completed():
    df = pd.DataFrame({
        "name": ["A","B"], "email": ["a@a.com","b@b.com"],
        "joined_date": ["2026-01-01","2026-01-02"],
        "country": ["SG","SG"], "bet_status": ["completed","completed"],
    })
    assert bet_completion_rate(df)["completion_rate_pct"] == 100.0
